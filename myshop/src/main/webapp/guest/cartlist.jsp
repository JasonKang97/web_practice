...
<table style="width:90%">
	...
	if(hCart.size()==0){	// hCart.isEmpty()도 사용 가능
	%>
	<tr>
	...
	<%
		...
		/*
		Enumeration enu = hCart.keys();	// Map 타입의 컬렉션을 읽어 Enumeration 객체를 이용해 반복 처리
		...
		*/
		...
		// for - each loop와 Map.Entry를 사용해 Hashtable의 key-value 쌍을 반복 처리 할 수 있다.
		// hCart.entrySet()은 Set을 반환하여 for-each loop에서 사용한다.
		// Map.Entry: Map에 저장된 모든 key-value 쌍을 각각의 key-value를 갖고 있는 하나의 객체로 얻는다.
		// 아래에 적힌 entry라는 객체 변수는 HashMap에 대한 key-value 쌍을 하나의 객체로 가지고 있어
		// getKey(). getValue() 메소드로 key와 value에 접근이 가능하다.
